    ...
    public class ConsoleCountriesSearchReportWriter
    {
        public void Write(CountryDto[] foundCountries) // zapis raportu na konsolę
        {
            ...
            foreach (var country in foundCountries)
            {
                ...
                {
                    double?[] fixedArray = { null, null };
                    country.LatLng = fixedArray;
                }

                Console.WriteLine($"\t\t- szerokość geograficzna: {(country.LatLng[0] > 0 ? country.LatLng[0].ToString() + $"{Convert.ToChar(176)}(N)" : (country.LatLng[0] == null ? null : country.LatLng[0].ToString() + $"{Convert.ToChar(176)}(S)"))}");
                Console.WriteLine($"\t\t- długość geograficzna:  {(country.LatLng[1] > 0 ? country.LatLng[1].ToString() + $"{Convert.ToChar(176)}(E)" : (country.LatLng[0] == null ? null : country.LatLng[0].ToString() + $"{Convert.ToChar(176)}(W)"))}");
